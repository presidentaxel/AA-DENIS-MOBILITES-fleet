...
def sync_trips(db: SupabaseDB, client: BoltClient, company_id: str | None = None, start: datetime | None = None, end: datetime | None = None, org_id: str | None = None, limit: int = 1000, offset: int = 0, incremental: bool = True) -> None:
    ...
    logger.info(f"Sync Bolt orders: company_id={company_id}, limit={limit}, offset={offset}, start_ts={start_ts}, end_ts={end_ts}")
    ...
    logger.info(f"Récupéré {len(orders)} orders depuis Bolt")
    
    if not orders:
        logger.warning("Aucun order récupéré depuis Bolt.")
        return
    ...
    try:
        # Récupérer les order_references existants pour cette période pour éviter les doublons
        ...
        logger.info(f"Vérification: {len(existing_order_refs)} orders déjà présents pour cette période")
        ...
        db.commit()
        logger.info(f"{saved_count} orders sauvegardés, {skipped_count} déjà présents (ignorés) avec org_id={org_id}")
        
    except Exception as e:
        db.rollback()
        logger.error(f"Erreur lors de la sauvegarde des orders: {e}", exc_info=True)
        raise
